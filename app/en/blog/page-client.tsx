...
import { blogPosts } from "@/app/en/blog/data" 
...
function BlogContent({
  ...
  const [activeCategory, setActiveCategory] = useState(() => {
    return initialCategory && posts.some((post: { category: string }) => post.category === initialCategory) ? initialCategory : "All"
  })
  ...
  const filteredPosts = useMemo(() => {
    return publishedPosts.filter((post: { title: string; description: string; category: string }) => {
      ...
      const matchesCategory = resolvedCategory === "All" || post.category === resolvedCategory
      ...
      <section className="relative min-h-[400px] pt-32 pb-20 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4">
          ...
          <p className="text-white/80 text-lg md:text-xl max-w-2xl mx-auto">
            Articles, guides, and resources on web development, digital marketing, and artificial intelligence
          </p>
        </div>
      </section>
              ...
              {columns.map((column, columnIndex) => (
                <div key={columnIndex} className="flex flex-col gap-6">
                  {column.map((post) => (
                    ...
                    >
                      {post.image && (
                        <Link href={`/en/blog/${post.slug}`} className="relative overflow-hidden block">
                          <Image
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-black">
                Ready to start your new business or project?
              </h2>
              <p className="mt-2 text-black/70">
                Tell us what you need and we will help you boost your project or business
              </p>
            </div>
...
export default function BlogPageEnClient() {
  return (
    <Suspense fallback={<Loading />}>
      <BlogContent initialSearchQuery="" initialCategory="All" />
    </Suspense>
  )
}